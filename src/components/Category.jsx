...
function Category() {
  ...
  const handlePrev = () => {
    setStartImageIndex(
      (startImageIndex - imagesPerPage + category.length) % category.length
    );
  };

  const handleNext = () => {
    setStartImageIndex((startImageIndex + imagesPerPage) % category.length);
  };
      ...
      <div className="px-4 md:px-36 py-4">
        <div className="justify-between flex my-4">
          <div className="text-2xl font-bold text-left">
            <h1>What's on mind?</h1>
          </div>
          <div className="flex gap-2">
            <button type="button" onClick={handlePrev} className="text-white bg-gray-300 hover:bg-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-full text-sm p-2.5 text-center inline-flex items-center me-2 dark:bg-gray-300 dark:hover:bg-gray-300 dark:focus:ring-gray-200">
